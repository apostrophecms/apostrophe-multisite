...
function exec(cmd) {
  return new Promise((resolve, reject) => {
    return require('child_process').exec(cmd, (err, stdout, stderr) => {
      console.log('>>', stdout);
      console.log('**', stderr);
      if (err) {
        return reject(err);
  ...
  construct: function(self, options) {
    self.beforeSave = function(req, doc, options, callback) {
      ...
      if (self.options.baseUrlDomains) {
        ...
        if ((typeof doc.shortName) === 'string') {
          doc.shortName = doc.shortName.toLowerCase();
        }
        if ((typeof doc.prodHostname) === 'string') {
          doc.prodHostname = doc.prodHostname.toLowerCase();
        }
    ...
    self.afterInsert = function(req, piece, options, callback) {
      ...
      async function copyContent(req, piece, options) {
        ...
        console.log('called copyContent');
        if (!piece.copyOfId) {
          return;
        }
        ...
        console.log('initial contents of new database home page after copy:');
        console.log(JSON.stringify(await self.apos.db.db(shortNamePrefix + piece._id).collection('aposDocs').findOne({ slug: '/', workflowLocale: 'default' }), null, '  '));
        console.log('getting from');
        const from = await getFrom();
        console.log('getting to');
        const to = await getTo();
        console.log('contents of new database home page after "getTo":');
        console.log(JSON.stringify(await self.apos.db.db(shortNamePrefix + piece._id).collection('aposDocs').findOne({ slug: '/', workflowLocale: 'default' }), null, '  '));
        console.log('purging trash');
        await purgeTrash();
        console.log('contents of new database home page after "purge trash":');
        console.log(JSON.stringify(await self.apos.db.db(shortNamePrefix + piece._id).collection('aposDocs').findOne({ slug: '/', workflowLocale: 'default' }), null, '  '));
        if (self.options.copy && self.options.copy.hardlink) {
          console.log('hardlinking attachments');
        ...
        async function copyDatabase() {
          ...
          if (compareVersions(info.version, '4.2') < 0) {
            console.log('running copydb command');
            const command = {
              copydb: 1,
            ...
            console.log('running mongodump pipeline');
            ...
            const cmd = shellQuote([ 'mongodump', `--uri=${fromUri}`, '--archive' ]) + ' | ' + shellQuote([ 'mongorestore', `--uri=${toUri}`, '--nsInclude=*', `--nsFrom=${fromDb}.*`, `--nsTo=${toDb}.*`, '--archive', '--drop' ]);
            console.log(cmd);
            return exec(cmd);
          }
        }
    ...
    self.apos.migrations.add('normalize-case', async () => {
      return self.apos.migrations.eachDoc({ type: 'site' }, async doc => {
        const $set = {};
        if ((typeof doc.shortName === 'string') && (doc.shortName !== doc.shortName.toLowerCase())) {
          const happy = await self.apos.docs.db.findOne({
            type: 'site',
            shortName: doc.shortName.toLowerCase()
          });
          if (happy) {
            // Avoid conflict with a site object already doing it right
            $set.shortName = doc.shortName.toLowerCase() + '-' + doc._id;
          } else {
            $set.shortName = doc.shortName.toLowerCase();
          }
        }
        if ((typeof doc.prodHostname === 'string') && (doc.prodHostname !== doc.prodHostname.toLowerCase())) {
          const happy = await self.apos.docs.db.findOne({
            type: 'site',
            prodHostname: doc.prodHostname.toLowerCase()
          });
          if (happy) {
            $set.prodHostname = doc.prodHostname.toLowerCase() + '-' + doc._id;
          } else {
            $set.prodHostname = doc.prodHostname.toLowerCase();
          }
        }
