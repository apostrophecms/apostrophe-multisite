...
module.exports = {
  ...
  beforeConstruct: function(self, options) {
    ...
    console.log('process.env', require('util').inspect(process.env, { colors: true, depth: 1 }))
    console.log('process.env.ENV', require('util').inspect(process.env.ENV, { colors: true, depth: 1 }))
    const baseDomain = options.baseUrlDomains[process.env.ENV || 'dev'];
    console.log('baseDomain', require('util').inspect(baseDomain, { colors: true, depth: 1 }))
  ...
  construct: function(self, options) {
    self.beforeSave = function(req, doc, options, callback) {
      ...
      if (self.options.baseUrlDomains) {
        ...
        if (shortName.match(/\:|\./)) {
          self.apos.notify(req, 'The short name of the site must not contain dots or a protocol. It is a short name like "nifty" (without quotes) <F2>and will be used as a "working name" for a temporary subdomain for your site until it is launched.', { type: 'error' });
        ...
        if (doc.prodHostname) {
          ...
          doc.prodBaseUrl = doc.prodHostname;
        }
      } else {
        // old behavior
